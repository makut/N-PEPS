...
from env.search import agg_and_beam_search, agg_and_cab
from model.encoder import TransformerEncoderBothWise
...
def train(configuration):
    ...
    val_problems = load_problems(params.val_path)

    # Define model
    model = PCCoder(lambda: configuration[0](**configuration[1]))
    if use_cuda:
        model.cuda()
    ...
    for epoch in range(params.num_epochs):
        ...
        with torch.no_grad():
            ...
            #         None, None, params.val_beam_size, params.val_beam_size, 12, None, None, 'none', None,
            #         device='cuda' if use_cuda else 'cpu'
            #     )
            #     count_success += top1_pred is not False
            # success_ratio = count_success / len(val_problems)
            # print('Success ratio: %f' % success_ratio)
...
CONFIGURATIONS = [
    # (TransformerEncoderBothWise, dict(n_layers=2, n_head=4, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=4, n_head=4, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=6, n_head=4, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=10, n_head=4, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=12, n_head=4, dim=128)),

    # (TransformerEncoderBothWise, dict(n_layers=2, n_head=8, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=4, n_head=8, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=6, n_head=8, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=10, n_head=8, dim=128)),
    # (TransformerEncoderBothWise, dict(n_layers=12, n_head=8, dim=128)),

    # (TransformerEncoderBothWise, dict(n_layers=4, n_head=8, dim=256)),
    # (TransformerEncoderBothWise, dict(n_layers=4, n_head=8, dim=512)),
    # (TransformerEncoderBothWise, dict(n_layers=4, n_head=8, dim=768)),
    (TransformerEncoderBothWise, dict(n_layers=4, n_head=8, dim=1024)),
]

if __name__ == '__main__':
    for configuration in CONFIGURATIONS:
        conf_string = configuration[0].__name__ + '_'.join(f'{key}={value}' for key, value in configuration[1].items())
        params.model_output_path = f'my/trained_models/E2/' + conf_string
        for i in range(5):
            print('Attempt', i)
            if train(configuration) is not False:
                break
